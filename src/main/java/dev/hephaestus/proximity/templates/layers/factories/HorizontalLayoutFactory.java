...
public class HorizontalLayoutFactory extends LayerFactory<HorizontalLayout> {
    private final List<LayerFactory<?>> factories;

    public HorizontalLayoutFactory(String id, int x, int y, List<CardPredicate> predicates, List<LayerFactory<?>> factories) {
        super(id, x, y, predicates);
        this.factories = new ArrayList<>(factories);
    }

    @Override
    public Result<HorizontalLayout> createLayer(String parentId, JsonObject card) {
        ...
        if (errors.isEmpty()) {
            return Result.of(new HorizontalLayout(parentId, this.id, this.x, this.y, layers));
        } else {
