...
public abstract class ParentFactoryFactory<T extends LayerFactory<?>> extends LayerFactoryFactory<T> {
    ...
    protected static Result<LayerFactoryFactory<?>> parse(Element element, String id, int x, int y, List<CardPredicate> predicates, Logger log, Function<String, CardPredicate> definedPredicateGetter, Factory factory) {
        List<LayerFactoryFactory<?>> factories = new ArrayList<>();
        List<String> errors = new ArrayList<>();

        XMLUtil.iterate(element, (e, i) -> {
            if (!e.getTagName().equals("conditions")) {
                Result<LayerFactoryFactory<?>> r = parse(e, x, y, log, definedPredicateGetter);
